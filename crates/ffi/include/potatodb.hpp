...
namespace potato {
    ...
    public:
        ...
        {
            return handle_ ? ::potato_result_get_column_type(handle_, col) : POTATO_TYPE_NULL;
        }
    ...
    public:
        ...
        ~StringList()
        {
            potato_string_list_free(handle_);
        }

        StringList(const StringList&)            = delete;
        StringList& operator=(const StringList&) = delete;
        StringList(StringList&& o) noexcept : handle_(o.handle_)
        {
            o.handle_ = nullptr;
        }
        ...
        {
            return potato_string_list_count(handle_);
        }
    ...
    public:
        ...
        ~IndexList()
        {
            potato_index_list_free(handle_);
        }

        IndexList(const IndexList&)            = delete;
        IndexList& operator=(const IndexList&) = delete;
        IndexList(IndexList&& o) noexcept : handle_(o.handle_)
        {
            o.handle_ = nullptr;
        }
        ...
        std::size_t count() const noexcept
        {
            return potato_index_list_count(handle_);
        }
    ...
    public:
        ...
        ~ResultList()
        {
            potato_result_list_free(handle_);
        }

        ResultList(const ResultList&)            = delete;
        ResultList& operator=(const ResultList&) = delete;
        ResultList(ResultList&& o) noexcept : handle_(o.handle_)
        {
            o.handle_ = nullptr;
        }
        ...
        std::size_t count() const noexcept
        {
            return potato_result_list_count(handle_);
        }
    ...
    public:
        ...
        ~QueryLog()
        {
            potato_query_log_free(handle_);
        }

        QueryLog(const QueryLog&)            = delete;
        QueryLog& operator=(const QueryLog&) = delete;
        QueryLog(QueryLog&& o) noexcept : handle_(o.handle_)
        {
            o.handle_ = nullptr;
        }
        ...
        std::size_t count() const noexcept
        {
            return potato_query_log_count(handle_);
        }
    ...
    public:
        ...
        ~Stream()
        {
            potato_stream_free(handle_);
        }

        Stream(const Stream&)            = delete;
        Stream& operator=(const Stream&) = delete;
        Stream(Stream&& o) noexcept : handle_(o.handle_)
        {
            o.handle_ = nullptr;
        }
    ...
    public:
        ...
        static Expected<Database> open(const std::string& data_dir)
        {
            ...
            if(!h) return Expected<Database>::err("Failed to open PotatoDB at: " + data_dir);
        ...
        Expected<Result> execute(const std::string& sql)
        {
            ...
            if(!raw)
            {
                ...
                return Expected<Result>::err(err ? std::string(err) : "Unknown query error");
            }
        ...
        Expected<Result> execute_prepared(const std::string& name, const std::vector<std::string>& params)
        {
            std::vector<const char*> c_params;
            c_params.reserve(params.size());
            for(auto& p: params) c_params.push_back(p.c_str());

            potato_result* raw = potato_execute_prepared(handle_, name.c_str(), c_params.data(), c_params.size());
            if(!raw)
            {
                const char* err = potato_last_error(handle_);
                return Expected<Result>::err(err ? std::string(err) : "Unknown query error");
        ...
        Expected<int64_t> parquet_file_count(const std::string& table_name)
        {
            ...
            {
                const char* err = potato_last_error(handle_);
                return Expected<int64_t>::err(err ? std::string(err) : "Unknown table stats error");
            }
        ...
        Expected<ResultList> execute_file(const std::string& path, bool continue_on_error = true)
        {
            potato_result_list* raw = potato_execute_file(handle_, path.c_str(), continue_on_error);
            if(!raw)
            {
                const char* err = potato_last_error(handle_);
                return Expected<ResultList>::err(err ? std::string(err) : "Failed to execute file");
            }
        ...
        Expected<Stream> execute_stream(const std::string& sql)
        {
            ...
            {
                const char* err = potato_last_error(handle_);
                return Expected<Stream>::err(err ? std::string(err) : "Unknown streaming error");
            }
        ...
        Expected<Result> recent_cdc()
        {
            ...
            {
                const char* err = potato_last_error(handle_);
                return Expected<Result>::err(err ? std::string(err) : "Failed to read recent CDC events");
            }
            return Expected<Result>(Result(raw));
        }
