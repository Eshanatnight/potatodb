...
impl<T> Clone for DistributionSender<T> {
    fn clone(&self) -> Self {
        self.channel.n_senders.fetch_add(1, Ordering::Relaxed);
...
impl<T> Drop for DistributionSender<T> {
    fn drop(&mut self) {
        let n_senders_pre = self.channel.n_senders.fetch_sub(1, Ordering::AcqRel);
...
impl<T> Future for SendFuture<'_, T> {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        let to_wake = {
            ...
            if this.gate.empty_channels.load(Ordering::Acquire) == 0 {
...
impl<T> Drop for DistributionReceiver<T> {
    fn drop(&mut self) {
        ...
        // decremented.
        if data.is_empty() && (self.channel.n_senders.load(Ordering::Acquire) > 0) {
            // channel is gone, so we need to clear our signal
            self.gate.decr_empty_channels();
        }
        ...
        match data.pop_front() {
            Some(element) => {
                ...
                if data.is_empty() && channel_state.recv_wakers.is_some() {
                    // update counter
                    let old_counter =
                        this.gate.empty_channels.fetch_add(1, Ordering::AcqRel);

                    // open gate?
                    let to_wake = if old_counter == 0 {
                        let mut guard = this.gate.send_wakers.lock();

                        // Mutex provides ordering; Relaxed is safe here.
                        if this.gate.empty_channels.load(Ordering::Relaxed) > 0 {
...
impl Gate {
    ...
    fn decr_empty_channels(&self) {
        let old_count = self.empty_channels.fetch_sub(1, Ordering::AcqRel);

        if old_count == 1 {
            let mut guard = self.send_wakers.lock();

            // Mutex provides ordering; Relaxed is safe here.
            if self.empty_channels.load(Ordering::Relaxed) == 0 && guard.is_none() {
                *guard = Some(Vec::new());
            }
        }
    }
